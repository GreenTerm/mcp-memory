...
$outputPath = Join-Path $artifactsDir "phase2_checks.txt"
...
@(
    "Running Phase 2 checks with $pythonCmd"
    "PYTHONPATH=$env:PYTHONPATH"
    ""
) | Set-Content -Encoding UTF8 $outputPath

& $pythonCmd -X utf8 -m unittest tests.test_api_server tests.test_gui_home -v *>&1 |
