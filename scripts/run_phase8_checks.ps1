...
$outputPath = Join-Path $artifactsDir "phase8_checks.txt"
...
@(
    "Running Phase 8 checks with $pythonCmd"
    "PYTHONPATH=$env:PYTHONPATH"
    ""
) | Set-Content -Encoding UTF8 $outputPath

& $pythonCmd -X utf8 -m unittest tests.test_api_server.ApiServerTests.test_ui_get_endpoints tests.test_api_server.ApiServerTests.test_ui_import_export_and_backup_flows -v *>&1 |
